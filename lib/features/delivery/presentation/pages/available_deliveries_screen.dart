...
class _AvailableDeliveriesScreenState extends State<AvailableDeliveriesScreen> {
  ...
  Future<void> _cancelOrder(int orderId) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(res['message']?.toString() ?? 'Delivery cancelled'), backgroundColor: Colors.green),
      );
      setState(() => _orders.removeWhere((o) => _orderId(o) == orderId));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _acceptOrder(int orderId) async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(res['message']?.toString() ?? 'Order accepted'), backgroundColor: Colors.green),
      );
...
class _OrderCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final vendorName = vendor is Map<String, dynamic> ? (vendor['name']?.toString() ?? '—') : '—';
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Expanded(child: Text(vendorName, style: TextStyle(fontSize: 14, color: Colors.grey.shade700))),
              ],
            ),
            ...
            if (itemCount > 0) ...[
              const SizedBox(height: 4),
              Text('$itemCount item${itemCount == 1 ? '' : 's'}', style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
            ],
            ...
            Row(
              ...
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Delivery fee', style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
                    Text('$deliveryFee', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
                  ],
                ),
                Column(
